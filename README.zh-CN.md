...
概览页是默认入口。它会把当前积灰风险放在每日温度和风扇卡片上方，并继续展示 24 小时和 7 天趋势。

![DustWatch 概览页](<screenshots/1. dashboard-overview.png>)

历史页用于探索完整记录。它支持原始、小时和日聚合视图，可以同时显示温度、风扇和 cooling loss 曲线，并在悬停时展示精确数值。

![历史页日聚合和悬停详情](<screenshots/2-2. history-panel-without-risk-purple-line.png>)

当演示数据或真实数据积累更久后，紫色 cooling loss 趋势可以帮助用户观察风险模型的信号如何随时间变化。

![历史页 30 天 cooling loss 趋势](<screenshots/3-2. demo-data-history-with-risk-purple-line.png>)
...
风险横条带有解释弹窗，所以应用不只是给出一个颜色，而是会说明为什么当前状态被判断为安全或有风险。

![无风险状态下的积灰风险解释弹窗](<screenshots/2-1. dashboard-overview-expanded-detail-explaining-why-there-is-no-risk.png>)

演示数据也可以用来查看高风险路径，不需要真的等待一台机器出现散热衰减。

![演示数据下的清灰建议和证据说明](<screenshots/3-1. demo-data-overview-expanded-with-risk.png>)
...
推送 `v*` tag 会触发 GitHub Actions release workflow。它会构建 arm64 和 x86_64 两个 DMG，并发布 GitHub Release 和自动生成的 release notes。`v0.*` 会标记为 prerelease，`v1.*` 及之后版本会作为正式 release 发布。
