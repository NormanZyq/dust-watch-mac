...
```sh
cd clean-notification-mac
./build.sh
cp -R build/CleanNotificationMac.app /Applications/
```

`build.sh` does three things:

1. `swift build -c release` compiles the binary
2. Assembles `CleanNotificationMac.app` (Info.plist + entitlements + binary)
...
1. Open `CleanNotificationMac.app` from Finder
...
3. Open **System Settings → Privacy & Security**, scroll down, click
   **Open Anyway** next to the CleanNotificationMac entry
...
```
~/Library/Application Support/CleanNotificationMac/data.db
```

A standard SQLite file. You can open it with `sqlite3` to inspect:

```sh
sqlite3 ~/Library/Application\ Support/CleanNotificationMac/data.db \
